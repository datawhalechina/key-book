...
以得到*经验风险与泛化风险的差距*（即 $\Phi(D)$ ) 在替换样本前后的风险能够被上界 $2\beta+M/m$ 控制住，根据 McDiarmid 不等式的描述，如果实值函数关于变量的替换如果具有较好的稳定性，那么该实值函数总有与期望的差距同样被上界控制住。（简单描述就是如果实值函数替换一个变量之后风险不大，那么无论怎么替换风险都不会过大，因此实值函数取值总会在一定范围内，从而总和均值（即期望）相差不大。）。因此在 (5.25) 处我们能够得到*经验风险与泛化风险的差距*（即 $\Phi(D)$ ) 同样有了上界，自然我们通过简单的放缩就可以获得一个常数上界，得到泛化风险的上界。
...
另外，在（5.32）的第一步推导中，利用到了绝对值不等式$E(X+Y)\le E(|X|)+E(|Y|)$。这种在期望缩放中运用绝对值不等式的处理方式在全书中并不少见，算是非常实用的小技巧了。
...
$$
参考**P95**对于$lim_{m\rightarrow +\infty}m\beta$讨论，发现只要满足$lim_{m\rightarrow +\infty}m\beta\lt\infty$这个条件，算法的泛化性能便可以得到保障，因此我们应确保$\beta$的取值不要太大。
在此定理中，我们选取$\beta=1/m$，此时（5.40）简化为：
$$
...
$$
其中，$\sim$ 表示当 $k$ 趋于无穷大时，两者的比值趋于1。
虽然该式的两边的差值会不断增加，但他们之间的百分比误差在稳定下降。
值得注意的是，Stirling 公式似即使在$k$很小的情况下也非常准确。
关于这个公式的证明可以参考[这里](https://en.wikipedia.org/wiki/Stirling%27s_approximation)，这里不做展开。

这里仅取 Stirling 公式中的一个变种格式：
$$
\sqrt{2\pi} k^{k+\frac{1}{2}} e^{-k} e^{\frac{1}{12k+1}} \le k! \le \sqrt{2\pi} k^{k+\frac{1}{2}} e^{-k} e^{\frac{1}{12k}}
$$

我们接下来套用 Stirling 公式，按照$k$的取值分为两种情况进行讨论。
当$k$为偶数时，二项式展开的最大项为：
$$
...
另外，在样本扰动的分析中，我们几乎没有对新增样本这一情况进行单独讨论。这是因为稳定性的要求在数据或者概念发生漂移的情况下不一定成立，因为这时候训练集的分布与真实分布已不再一致。而本章在研究训练集$D$的扰动对算法$\mathfrak{L}_D$输出函数的影响时，希望经验风险的变化尽可能地小，这恰好与在线学习（Online Learning）的目标有一些抵触。
具体而言，在线学习指的是在数据不断到来的过程中，动态地更新模型，因而该训练方式更关注模型的可塑性，即在旧场景中训练的模型是否能通过优化在新场景中表现优异，因此我们在真实场景中需要平衡学习算法的可塑性与稳定性。为了更好地评估在线学习的性能，本书引入了遗憾界的概念，即在线学习与离线学习算法之间最小损失的差值，具体分析参考第八章。
