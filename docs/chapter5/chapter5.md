...
另外，在样本扰动的分析中，我们几乎没有对新增样本这一情况进行单独讨论。这是因为稳定性的要求在数据或者概念发生漂移的情况下不一定成立，因为这时候训练集的分布与真实分布已不再一致。而本章在研究训练集$D$的扰动对算法$\mathfrak{L}_D$输出函数的影响时，希望经验风险的变化尽可能地小，这恰好与[持续学习](https://en.wikipedia.org/wiki/Continuous_training)的目标有一些抵触。
