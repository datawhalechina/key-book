...
&= \frac{1}{m} \sum_{i=1}^{m} \mathbb{E}_{(\boldsymbol{x_i}, y_i) \sim \mathcal{D}}[{ \mathbb{I}\left(y_{i} f\left(\boldboldsymbol{x}_{i}\right) \leqslant 0\right)}]\\
...
**P124**的定理6.2给出了划分一致性的充分条件。首先我们定义了 $\Omega(\boldsymbol{x})$ 作为划分区域的条件概率极大似然估计量：
$$
\hat{\eta}(\boldsymbol{x})=\sum_{\boldsymbol{x}_{i} \in \Omega(\boldsymbol{x})} \frac{\mathbb{I}\left(y_{i}=+1\right)}{N(\boldsymbol{x})}
$$
再根据划分机制构造分类器（输出函数）$h_{m}(\boldsymbol{x})=2 \mathbb{I}\left(\hat{\eta}(\boldsymbol{x}) \geqslant \frac{1}{2}\right)-1$。为了证明划分机制的一致性，我们需要证明其输出函数的泛化风险在 $m$ 趋于无穷时，趋于贝叶斯风险。

在此，我们利用了基于条件概率估计的插值法，并借助引理6.2得到了输出函数的泛化风险与贝叶斯风险之间的差值不等式。对于不等式右侧的期望，利用三角不等式进行放缩，可得（6.62）。

根据假设条件：
$$
\lim _{m \rightarrow \infty} P((Diam(\Omega)-0) \geq \epsilon) =  \lim _{m \rightarrow \infty} P(( \sup _{\boldsymbol{x}, \boldsymbol{x}^{\prime} \in \Omega}\left\|\boldsymbol{x}-\boldboldsymbol{x}^{\prime}\right\| -0) \geq \epsilon)=0
$$

由于 $\eta(x)$ 在样本空间中具有连续性，因此在任意邻域内我们都可以用 $\hat\eta(x)$ 的期望值来近似 $\eta(x)$。当邻域趋于 0 时，可得：
$$
\mathbb{E}[|\bar{\eta}(\boldboldsymbol{x})-\eta(\boldsymbol{x})|] \rightarrow 0
$$
这是由于 ${x}^{\prime}$ 被依概率限制在一个 $\epsilon$ 邻域内，且期望可以忽略概率趋于 0 的点，因此 $\bar{\eta}(\boldsymbol{x})$ 由于 $\eta(x)$ 的连续性也被限制在一个 $\eta(x)$ 的 $\epsilon$ 邻域内，从而期望的极限得证。
...
\mathbb{E}\left[|\hat{\eta}(\boldboldsymbol{x})-\bar{\eta}(\boldsymbol{x})|\mid N(x)=0 , \boldsymbol{x}, \boldsymbol{x}_{1}, \ldots, \boldboldsymbol{x}_{m}\right] \\
+\mathbb{E}\left[\left|\sum_{\boldsymbol{x}_{i} \in \Omega(\boldsymbol{x})} \frac{\mathbb{I}\left(y_{i}=+1\right)-\bar{\eta}(\boldboldsymbol{x})}{N(\boldsymbol{x})}\right| N(\boldboldsymbol{x})>0, \boldsymbol{x}, \boldsymbol{x}_{1}, \ldots, \boldsymbol{x}_{m}\right]\\
\leqslant P\left(N(\boldsymbol{x})=0 \mid \boldboldsymbol{x}, \boldsymbol{x}_{1}, \ldots, \boldboldsymbol{x}_{m}\right) + \mathbb{E}\left[\left|\sum_{\boldboldsymbol{x}_{i} \in \Omega(\boldsymbol{x})} \frac{\mathbb{I}\left(y_{i}=+1\right)-\bar{\eta}(\boldsymbol{x})}{N(\boldsymbol{x})}\right| N(\boldsymbol{x})>0, \boldsymbol{x}, \boldsymbol{x}_{1}, \ldots, \boldsymbol{x}_{m}\right]
\end{array}
$$
然后，对于不等式右侧的第二部分，利用引理6.3的不等式，可以得到：
$$
\begin{array}{l}
\mathbb{E}\left[\left|\sum_{\boldboldsymbol{x}_{i} \in \Omega(\boldboldsymbol{x})} \frac{\mathbb{I}\left(y_{i}=+1\right)-\bar{\eta}(\boldsymbol{x})}{N(\boldboldsymbol{x})}\right| N(\boldsymbol{x})>0, \boldsymbol{x}, \boldsymbol{x}_{1}, \ldots, \boldsymbol{x}_{m}\right] \\
\leqslant \mathbb{E}\left[\sqrt{\frac{\bar{\eta}(\boldboldsymbol{x})(1-\bar{\eta}(\boldboldsymbol{x}))}{N(\boldsymbol{x})}} \mathbb{I}(N(\boldboldsymbol{x})>0) \mid \boldboldsymbol{x}, \boldboldsymbol{x}_{1}, \ldots, \boldboldsymbol{x}_{m}\right]
\end{array}
$$

对于此不等式的右侧，再进行放缩。对于任意 $k \geq 3$，当 $N(x) \leqslant k$ 时，$\sqrt{\frac{\bar{\eta}(\boldsymbol{x})(1-\bar{\eta}(\boldsymbol{x}))}{N(\boldboldsymbol{x})}} \leqslant \frac{1}{2}$，当 $N(x) > k$ 时，$\sqrt{\frac{\bar{\eta}(\boldboldsymbol{x})(1-\bar{\eta}(\boldboldsymbol{x}))}{N(\boldboldsymbol{x})}} \leqslant \frac{1}{2\sqrt k}$，从而得到不等式右侧的进一步放缩：
$$
\leqslant \frac{1}{2} P\left(N(\boldboldsymbol{x}) \leqslant k \mid \boldboldsymbol{x}, \boldboldsymbol{x}_{1}, \ldots, \boldboldsymbol{x}_{m}\right)+\frac{1}{2 \sqrt{k}} P\left(N(\boldboldsymbol{x}) > k \mid \boldboldsymbol{x}, \boldboldsymbol{x}_{1}, \ldots, \boldboldsymbol{x}_{m}\right)\\
\leqslant \frac{1}{2} P\left(N(\boldboldsymbol{x}) \leqslant k \mid \boldboldsymbol{x}, \boldboldsymbol{x}_{1}, \ldots, \boldboldsymbol{x}_{m}\right)+\frac{1}{2 \sqrt{k}}
$$
...
$$
\mathbb{E}[|\hat{\eta}(\boldboldsymbol{x})-\bar{\eta}(\boldboldsymbol{x})|] \rightarrow 0
$$
最终证明了其输出函数的泛化风险在 $m$ 趋于无穷时，趋于贝叶斯风险：
$$
R\left(h_{m}\right)-R^{*} \leqslant 2 \mathbb{E}[|\hat{\eta}(\boldboldsymbol{x})-\eta(\boldboldsymbol{x})|]  \rightarrow 0
$$
