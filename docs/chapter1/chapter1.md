...
对于函数$f(x)$，若 $exp(f(x))$ 是凹函数，则称其为指数凹函数。注意到，当$exp(f(x))$ 是凹函数时，$f(x)$ 不一定是凹函数。
...
3. 加法性质：如果 $f(x)$ 和 $g(x)$ 都是指数凹函数，那么他们的和 $f(x)+g(x)$ 也是指数凹函数。
4. 乘法性质：如果 $f(x)$ 是指数凹函数且 $h(x)$ 是非负的凹函数，那么他们的乘积 $f(x)h(x)$ 也是指数凹函数。
...
在数学分析中，函数的光滑性是通过其在某个定义域上具有的连续导数的数量来衡量的性质，这个性质称为可微性类。
...
次梯度是凸函数导数的一种推广形式，某些凸函数在特定区域内导数可能并不存在，但我们依旧可以用次梯度来表示此区域内函数变化率的下界。
