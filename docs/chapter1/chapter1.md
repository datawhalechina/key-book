...
特别的，$p = q = 2 $ 时，Holder不等式退化为Cauchy-Schwarz 不等式：
$$
...
取 $Y=e^{\lambda X},$ 应用Markov 不等式即得证。
...
### 随机变量的次高斯性
上述使用Markov不等式的技术称为Chernoff 界的一般技巧，得到的界称之为Chernoff Bound。其核心即是对其矩母函数进行控制。于是有定义：
...
实际上Hoeffding引理中的随机变量$X$服从$\frac{(b-a)}{2}$的次高斯分布，Hoeffding引理也是次高斯分布的直接体现。次高斯性还有一系列等价定义方式，这里不是本笔记讨论的重点。
...
常见的次指数分布包括:指数分布，Gamma分布，以及**任何的有界随机变量**。
...
又因为 $ X_{i}$ 为鞅差序列，则 $ \mathbb{E}\left[X_{m} \mid \mathcal{F}_{m-1}\right]=0, \mathbb{E}\left[X_{i}\right]=0$ ，再结合不等式$\mathbb{E}\left[e^{\lambda X} \mid Y\right] \leq \mathbb{E}\left[e^{\lambda X}\right]$及Hoeffding引理，有：
$$
\begin{aligned}
...
$$
且 $f$ 为差有界的，则满足Azuma-Hoeffding 引理，代入则得到：
$$
...
## Theorem 15: Johnson-Lindenstrauss引理
...
$$
类似地使用Chernoff 一般性技巧，在$\lambda = \varepsilon/3$时可以证得左端不等式。
对于右端不等式，考察矩母函数$\forall \lambda < 1/2$：
$$
	\mathbb{E}[e^{\lambda X^2}] = (1-2\lambda)^{-m/2}
$$
再次使用Chernoff 一般性技巧，取$\lambda = \varepsilon/6$即可得证。
