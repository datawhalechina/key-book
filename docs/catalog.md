...
神经网络的发展历史为这一观点提供了重要例证。1987 年，Robert Hecht-Nielsen 证明，具有单隐层结构的前馈神经网络可以逼近任意连续的多元函数。这一结果后来被称为通用逼近定理（Universal Approximation Theorem, UAT），其核心含义在于：在表示能力层面，神经网络并不受限于简单函数族。随后，Vugar Ismailov 等人的工作进一步表明，即便是某些不连续函数，也可以通过类似结构进行逼近。这些结果在理论上极大拓展了神经网络的表达能力边界，但它们回答的是“是否存在某种参数配置”，而非“是否能够通过有限数据和可行的学习过程找到该配置”。通用逼近定理因此既是深度学习成功的重要思想源头，也是误解最为频繁的理论结果之一。
