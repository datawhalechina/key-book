...
双下降现象描述的是**测试误差**随模型复杂度变化呈现两次下降的非单调曲线，由三个阶段组成：
...
双下降现象表明，在过参数化的深度学习模型中，传统的"模型越大越容易过拟合"的认知并不总是成立。理解这一现象有助于更好地选择模型规模，并结合正则化和数据增广等策略实现最佳泛化性能。更多实验细节参考文献：[Deep Double Descent: Where Bigger Models and More Data Hurt](https://iopscience.iop.org/article/10.1088/1742-5468/ac3a74)。
